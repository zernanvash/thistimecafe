    ...
    return (
        <main className="min-h-screen flex items-center justify-center p-3 sm:p-4 bg-[var(--bg)] font-sans">
            <section className="w-full max-w-[460px] bg-[var(--surface)] border border-[var(--border)] rounded-[18px] sm:rounded-[24px] shadow-[var(--shadow)] overflow-hidden">
                <header className="px-5 sm:px-7 py-5 sm:py-6 border-b border-[var(--border)]">
                    <BrandMark className="login-brand" />
                </header>

                <div className="p-5 sm:p-7 space-y-5">
                    <section className="rounded-[22px] border border-[var(--border)] bg-[color-mix(in_oklch,var(--accent)_9%,var(--surface))] p-5 sm:p-6 shadow-[0_12px_26px_color-mix(in_oklch,var(--fg)_8%,transparent)]">
                        <h2 className="text-2xl font-display font-bold text-[var(--fg)]">Cashier PIN</h2>
                        <div aria-label={`${pin.length} of 6 PIN digits entered`} className="mt-5 min-h-[92px] rounded-[20px] flex items-center justify-center gap-3 bg-[var(--surface)] border border-[color-mix(in_oklch,var(--accent)_28%,var(--border))]">
                            {[0, 1, 2, 3, 4, 5].map(idx => (
                                <span
                                    key={idx}
                                    className={`w-5 h-5 border-2 border-[var(--border)] rounded-full transition-all ${
                                        pin.length > idx ? 'bg-[var(--fg)] border-[var(--fg)] scale-110' : ''
                                    }`}
                                />
                            ))}
                        </div>
